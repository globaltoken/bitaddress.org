ninja.wallets.vanitywallet = {
	...
	generateKeyPair: function () {
		...
		var privateKey = key.getGlobaltokenHexFormat();
	...
	addKeys: function () {
		...
		var globaltokenAddress = ninja.translator.get("vanityinvalidinputcouldnotcombinekeys");
		...
		try {
			...
			if (ninja.publicKey.isPublicKeyHexFormat(input1KeyString) && ninja.publicKey.isPublicKeyHexFormat(input2KeyString)) {
				...
				if (document.getElementById("vanityradioadd").checked) {
					...
					if (pubKeyByteArray == null) {
						...
						globaltokenAddress = ninja.publicKey.getGlobaltokenAddressFromByteArray(pubKeyByteArray);
			...
			// one public key and one private key
				...
				var ecKey = (ninja.privateKey.isPrivateKey(input1KeyString)) ? new Globaltoken.ECKey(input1KeyString) : new Globaltoken.ECKey(input2KeyString);
				// add 
				if (document.getElementById("vanityradioadd").checked) {
				...
				if (pubKeyCombined == null) {
					...
					globaltokenAddress = ninja.publicKey.getGlobaltokenAddressFromByteArray(pubKeyCombined);
			...
			// both inputs are private keys
				...
				if (combinedPrivateKey == null) {
					...
					globaltokenAddress = combinedPrivateKey.getGlobaltokenAddress();
					privateKeyWif = combinedPrivateKey.getGlobaltokenWalletImportFormat();
		...
		document.getElementById("vanityaddress").innerHTML = globaltokenAddress;
