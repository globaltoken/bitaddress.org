ninja.wallets.splitwallet = {
	...
	splitKey: function () {
		try {
			...
			var shares = ninja.wallets.splitwallet.getFormattedShares(key.getGlobaltokenHexFormat(), numshares, threshold);

			var output = document.createElement("div");
			output.setAttribute("id", "splitoutput");
			var m = {};
			output.appendChild(this.mkOutputRow(globaltokenAddress, "split_addr", "Globaltoken Address:    "));
			m["split_addr"] = globaltokenAddress;
	...
	combineShares: function () {
		try {
			...
			var privkeyBase58 = new Globaltoken.ECKey(combinedBytes).getGlobaltokenWalletImportFormat();
	...
	getFormattedShares: function (key, numshares, threshold) {
		var shares = secrets.share(key, numshares, threshold).map(ninja.wallets.splitwallet.hexToBytes).map(Globaltoken.Base58.encode);
		return shares;
	},

	// combine base58 formatted shares and return a globaltoken byte array
	combineFormattedShares: function (shares) {
		var combined = secrets.combine(shares.map(Globaltoken.Base58.decode).map(Crypto.util.bytesToHex).map(ninja.wallets.splitwallet.stripLeadZeros));
